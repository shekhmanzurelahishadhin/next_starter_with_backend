...
const DropzoneComponent: React.FC<DropzoneProps> = ({ multiple = true }) => {
  ...
  const onDrop = (acceptedFiles: File[]) => {
    ...
    console.log("Dropped files:", mapped);
    // If multiple false → only 1 file preview allowed
    if (!multiple) {
      setFiles([mapped[0]]);
    } else {
    ...
    <ComponentCard title="Dropzone">
      <div className="transition border border-gray-300 border-dashed cursor-pointer dark:hover:border-brand-500 dark:border-gray-700 rounded-xl hover:border-brand-500">
        ...
        >
          ...
          {/* ---------------------------------------------------
              If files exist → PREVIEW APPEARS INSIDE THE BOX
             --------------------------------------------------- */}
          {files.length > 0 ? (
